...
if (-not $SkillVersion) {
    Fail-Package "manifest version is empty"
}
if (-not $MinVersion) {
    Fail-Package "manifest cli.min_version is empty"
}

$CliHome = if ($env:ZHIHU_CLI_HOME) {
    $env:ZHIHU_CLI_HOME
} else {
    Join-Path $env:LOCALAPPDATA "ZhihuCLI"
}
...
function Compare-SemVer([string]$A, [string]$B) {
    ...
    for ($PartIndex = 0; $PartIndex -lt $PartCount; $PartIndex++) {
        ...
        $Comparison = [string]::CompareOrdinal($AIdentifier, $BIdentifier)
